...
public class GameplayHand : BaseHand
{
    ...
    [SerializeField] private XRBaseInteractor _targetInteractor = null;
    private void OnEnable()
    {
        // Subscribe to selected events
        _targetInteractor.onSelectEntered.AddListener(TryApplyObjectPose);
        _targetInteractor.onSelectExited.AddListener(TryApplyDefaultPose);
    }

    private void OnDisable()
    {
        // Unsubscribe to selected events
        _targetInteractor.onSelectEntered.RemoveListener(TryApplyObjectPose);
        _targetInteractor.onSelectExited.RemoveListener(TryApplyDefaultPose);
    }
    ...
    public override void ApplyOffset(Vector3 position, Quaternion rotation)
    {
        ...
        _targetInteractor.attachTransform.localPosition = finalPosition;
        _targetInteractor.attachTransform.localRotation = finalRotation;
    }

    private void OnValidate()
    {
        // Let's have this done automatically, but not hide the requirement
        if(!_targetInteractor)
            GetComponentInParent<XRBaseInteractor>();
    }
}